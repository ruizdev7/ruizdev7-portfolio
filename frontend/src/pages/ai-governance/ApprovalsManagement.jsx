...
const ApprovalsManagement = () => {
  ...
  const { data: approvalsData, isLoading, error, refetch } = useGetApprovalsQuery({
    assigned_only: true,
    status: statusFilter,
  });
  ...
  const onSubmitAction = async (data) => {
    ...
    try {
      ...
      toast.error(error?.data?.error || `Error al ${actionType === 'approve' ? 'aprobar' : 'rechazar'} la tarea`);
    }
  };
  ...
  const isOverdue = (approval) => {
    ...
    return new Date(approval.deadline) < new Date() && approval.status === 'pending';
  };
        ...
        {approvals.length === 0 ? (
          <div className="col-span-full text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              No hay aprobaciones {statusFilter === 'pending' ? 'pendientes' : statusFilter === 'approved' ? 'aprobadas' : 'rechazadas'}
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded ${getStatusBadge(approval.status)}`}>
                      {approval.status === 'pending' && <ClockIcon className="h-4 w-4" />}
                      {approval.status === 'approved' && <CheckCircleIcon className="h-4 w-4" />}
                      {approval.status === 'rejected' && <XCircleIcon className="h-4 w-4" />}
                      {approval.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {approval.priority && (
                    <div>
                      <span className="font-medium">Prioridad:</span> {approval.priority}
                    </div>
                  )}
                </div>
                ...
                {approval.task_output && (
                  <div className="mb-4">
                    ...
                    <div className="p-2 bg-gray-50 dark:bg-gray-900 rounded text-xs text-gray-900 dark:text-white max-h-32 overflow-y-auto">
                      {typeof approval.task_output === 'string'
                        ? approval.task_output
                        : JSON.stringify(approval.task_output, null, 2)}
                    </div>
                  </div>
                )}
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded ${getStatusBadge(selectedApproval.status)}`}>
              ...
              {selectedApproval.task_output && (
                <div>
                  ...
                  <pre className="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg text-sm text-gray-900 dark:text-white overflow-x-auto">
                    {typeof selectedApproval.task_output === 'string'
                      ? selectedApproval.task_output
                      : JSON.stringify(selectedApproval.task_output, null, 2)}
                  </pre>
                </div>
              )}
              ...
              {selectedApproval.modified_output && (
                <div>
                  ...
                  <pre className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm text-gray-900 dark:text-white overflow-x-auto">
                    {typeof selectedApproval.modified_output === 'string'
                      ? selectedApproval.modified_output
                      : JSON.stringify(selectedApproval.modified_output, null, 2)}
                  </pre>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                {selectedApproval.approved_at && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                      {selectedApproval.status === 'approved' ? 'Aprobado' : 'Rechazado'}
                    </label>
                    <p className="text-sm text-gray-900 dark:text-white">
      ...
      {isActionModalOpen && selectedApproval && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-700 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                {actionType === 'approve' ? 'Aprobar Tarea' : 'Rechazar Tarea'}
              </h2>
              <button
                onClick={closeModals}
                className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
            ...
            <form onSubmit={handleSubmit(onSubmitAction)} className="p-6 space-y-4">
              {actionType === 'approve' && (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm text-blue-800 dark:text-blue-300">
                        Opcionalmente puedes modificar el output antes de aprobar. Si no proporcionas un output modificado, se aprobará el output original.
                      </p>
                    </div>
                  </div>
                </div>
