...
export const aiGovernanceApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    markTaskFailed: builder.mutation({
      query: (taskId) => ({
        url: `/ai/tasks/${taskId}/mark-failed`,
        method: "POST",
      }),
    ...
    executeTaskStream: builder.mutation({
      queryFn: async (taskData, { getState }) => {
        ...
        if (!response.body) {
          return { error: { error: "No response body" } };
        }
    ...
    getApproval: builder.query({
      ...
      providesTags: (result, error, approvalId) => [
    ...
    getApprovalSettings: builder.query({
      query: () => "/ai/approval-settings",
      providesTags: ["Policy"],
    }),

    updateApprovalSettings: builder.mutation({
      query: (settingsData) => ({
        url: "/ai/approval-settings",
        method: "PUT",
        body: settingsData,
      }),
      invalidatesTags: ["Policy", "Dashboard"],
    }),
